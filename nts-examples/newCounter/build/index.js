const exposed = (() => {
    ...
    function _getBalance(mint, address) {
        ...
        if (mint.trim() === "")
            throw new Error("Mint address cannot be empty");
        if (address.trim() === "")
    ...
    function _withdraw(amount, mint, destinationAppId, receiverOpt, senderOpt) {
        ...
        nsend(destinationAppId, mint, withdrawalAmount.toString(), { receiver });
    ...
    function _withdrawToCluster(amount, mint, destinationClusterId, receiverOpt, senderOpt) {
        ...
        nwrite("user_balance_" + mint + "_" + sender, (prevAmount - withdrawalAmount).toString(), "user_balance_" + sender);
        nsendToCluster(destinationClusterId, mint, withdrawalAmount.toString(), { receiver });
    ...
    function _mintTransfer(amount, mint, destination) {
        ...
        __deposit(amount, mint, JSON.stringify({ receiver: destination }));
    ...
    class NMap {
        ...
        get(key) {
            ...
            const value = this.parent.kvRead(`${this.prefix}:${key}`);
            if (value !== undefined) {
        ...
        has(key) {
            return this.data.has(key) || this.parent.kvRead(`${this.prefix}:${key}`) !== undefined;
        }
        clear(tag) {
    ...
    class NApp {
        ...
        addPendingChange(key, value, tag) {
            this.pendingChanges.set(key, { value, tag });
        }
        kvRead(key) {
    ...
    class TransferNApp extends NApp {
        withdrawAction(amount, mint, destinationAppId, receiverOpt) {
            withdraw(amount, mint, destinationAppId, receiverOpt);
        }
        withdrawToClusterAction(amount, mint, destinationClusterId, receiverOpt) {
            withdrawToCluster(amount, mint, destinationClusterId, receiverOpt);
        }
        mintTransferAction(amount, mint, destination) {
            mintTransfer(amount, mint, destination);
        }
        mintAction(totalSupply, admin, meta) {
            mint(totalSupply, admin, meta);
        }
        transferAction(amount, mint, receiver) {
            transfer(amount, mint, receiver);
        }
    }
    NApp.injectKVMethods((key) => nread(key), (key, value, tag) => nwrite(key, value, tag));
    TransferNApp.injectKVMethods((key) => nread(key), (key, value, tag) => nwrite(key, value, tag));
    function boum() {
    ...
    const { increment, decrement } = createExecutableFunctions(Counter);
